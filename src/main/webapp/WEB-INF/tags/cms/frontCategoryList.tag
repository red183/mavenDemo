   		...
   		<c:choose>
   			<c:when test="${not empty tpl.href}">
    			<c:choose>
	    			...
	    			<c:set var="url" value="${ctxFront}${tpl.href}"/></c:when><c:otherwise><c:set var="url" value="${tpl.href}"/></c:otherwise>
	    		</c:choose>
   			</c:when>
   			<c:otherwise><c:set var="url" value="${ctxFront}/list-${tpl.id}${urlSuffix}"/></c:otherwise>
   		</c:choose>
