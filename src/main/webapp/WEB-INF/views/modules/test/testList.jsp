...
<head>
	...
	<script type="text/javascript">
		...
		function add(){
			alertx("新建数据");
			location = "${ctxAdmin}/test/test/form";
		}
		...
		function edit(){
			...
			if (id != ""){
				location = "${ctxAdmin}/test/test/form?id=" + id;
				alertx("修改数据：id=" + id);
			}else{
				alertx("请选择一行数据！");
			}
		...
		function dele(){
			...
			if (ids != ""){
				$.get("${ctxAdmin}/test/test/delete?id=" + ids, function(data){
					if (data == "true"){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxAdmin}/test/test/">硕正列表</a></li>
		<shiro:hasPermission name="test:test:edit"><li><a href="${ctxAdmin}/test/test/form">组件表单</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="test" action="${ctxAdmin}/test/test/listData.json" method="post" class="breadcrumb form-search hide" onsubmit="return page();">
